...
import com.mildlyskilled.Coordinator.{PrintFinished, Print}
...
class Scene private(val objects: List[Shape], val lights: List[Light]) {
  ...
  def traceImage(width: Int, height: Int, shutdownFn: () => Unit)(implicit executionContext: ExecutionContext, timeout: Timeout, coordinatorActor: ActorRef) {
    ...
    val generatingPixels = Future {

      val pixelCoordinates = (0 until height).flatMap { y =>
        (0 until width).map(x => (y, x) )
      }

      pixelCoordinates.par.foreach { case (y, x) =>
        ...
        coordinatorActor ! Pixel(x, y, colour)
      }
    }

    generatingPixels.onComplete {
      case Success(_) => {
        (coordinatorActor ? Print).onComplete {
          case Success(_) => shutdownFn()
          case Failure(e) => {
            shutdownFn()
            throw new RuntimeException("Failed to print image")
          }
        }

      }
      case Failure(e) => throw new RuntimeException("Failed to generate all pixels")
    }
  }
